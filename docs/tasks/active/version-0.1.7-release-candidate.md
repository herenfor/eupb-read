...
- 状态：隔离副本已整理，待 Windows 编译分发与实机验收
- 创建日期：2026-08-20
- 最后更新：2026-08-20
- 对应版本：0.1.7
...
- 本测试版包含 B-023～B-030 渲染/交互修复、B-031 链接书库重构、B-032 桌面单实例、B-033 三步前进/后退历史、B-034 顶层 float 版心兼容和 B-035 图片脚注 marker 修复。
- 不纳入相邻章节预加载、动画、全文搜索、高亮和多个自定义 CSS 预设。

## 已完成的本地验证

- 前端 Vitest：23 个文件、228/228。
- TypeScript：`tsc --noEmit` 通过。
- Vite production build：通过。
- Rust：10/10、`cargo fmt --check`、`cargo check --locked` 通过；`cargo metadata --locked --no-deps` 识别 `epub-reader@0.1.7`。
