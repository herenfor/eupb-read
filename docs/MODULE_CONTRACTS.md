...
- 阅读进度写入必须保证同一本书最后稳定位置胜出：UI 先更新内存态，后台写入单通道串行并合并待写值；返回书架和 Tauri 关闭窗口前必须 flush。`markOpened` 只能修改 `isNew`，不能用陈旧整条记录覆盖进度。
...
- 所有 Tauri 索引变更必须经同一写入互斥；正文和封面先写同目录暂存文件，提交索引时一次完成，不得用 JSON 数字数组传输大型二进制。
- 精确重复以 EPUB 原始字节 SHA-256 判断。新书可以用指纹作为 ID；0.1.5 旧 ID/目录保持不变，缺失的 `contentHash` 只在同大小候选参与判重时懒补。
- 重复项在解析/写盘前尽早跳过，存储提交层仍须按 `contentHash` 复检。内容不同默认作为新书，不隐式覆盖或迁移进度。
