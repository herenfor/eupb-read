...
- 分页器对 viewer 直接 `reader-top` 的 computed `left/right` float，可在解除阅读器 auto margin 后确认作者两侧无 meaningful 水平 margin、元素 border-box 宽度不超过 40rem 且不存在全页/全宽突破意图时，把物理 float 侧 margin 写为 `max(0,(parentWidth-min(parentWidth,40rem))/2)`，另一侧为 0；窄容器写入 0。该写回必须进入既有 `marginFixes`，下一轮 measure/dispose 恢复原 inline 值及 priority；作者 margin、过宽或明确 breakout 设计不得被覆盖。
...
- 阅读位置至少包含章节、页码和可选内容锚点；初始/历史恢复有 anchor 时只使用 anchor，只有 anchor=null 才使用页码兜底；转场 pending/display gate 未完成时不得把旧 ready 状态写成新章节进度。
...
- 阅读进度保持“最后稳定值胜出”：前端合并连续更新，返回书架、隐藏和正常关闭时 flush；`markOpened` 只能改变 `isNew`。
