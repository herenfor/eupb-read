...
- 自动去重（同一标识、文件名与大小视为同一本书）
- 封面网格（无封面时生成书名占位封面），封面比例按台版轻小说常见 128:188
...
- 单本删除与批量删除均带二次确认
- Tauri 环境书籍持久化到系统应用数据目录；浏览器开发环境使用 IndexedDB 回退
...
- **持久化**：WebView `localStorage`（设置/进度）+ 应用数据目录或 IndexedDB（书架）
...
src/
  ...
  ui/          界面
    ...
    shelf.ts        书架存储抽象（Tauri + IndexedDB）
    storage.ts      设置与阅读进度
  test/        测试夹具与单元测试
src-tauri/    Tauri 2 壳
scripts/      构建/自检工具
...
浏览器开发模式下，书架使用 IndexedDB 持久化；其余功能与桌面端一致。
